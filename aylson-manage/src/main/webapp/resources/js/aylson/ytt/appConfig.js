	...
	$(function() { 
		datagrid = $('#datagrid').datagrid({
			method:'get',
			url : projectName+'/ytt/appConfig/admin/list?v_date=' + new Date(),
			pagination : true,
			pageSize : 20,
			...
			buttons:[{
			    ...
			    handler:function(){
				    	$("#appConfigConfigForm").form('submit',{
				    		 type:'POST',
				    		 url : projectName+'/ytt/appConfig/admin/add',
	...
	function query(id){
		win = $("<div></div>").dialog({
			...
			maximizable:true,
			modal:true,
			href:projectName+'/ytt/appConfig/admin/toEdit?id='+id,
			onClose:function(){
		    		$(this).dialog("destroy");
		    },
			buttons:[{
	...
	function del(id){
		$.messager.confirm("提示","确定删除此记录吗？",function(r){
			if(r){
				$.ajax({
					type:"POST",
					url:projectName+'/ytt/appConfig/admin/deleteById?id=' + id,
					dataType:"json",
	...
	function changeStatus(id, status){
		...
		$.messager.confirm("提示",tip,function(r){
			if(r){
				$.ajax({
					type:"POST",
					url:projectName+'/ytt/appConfig/admin/changeStatus?id=' + id+'&status='+status,
					dataType:"json",
					success:function(data){
						if(data){
		    					$.messager.show({"title":"系统提示","msg":data.message,"timeout":1000});
