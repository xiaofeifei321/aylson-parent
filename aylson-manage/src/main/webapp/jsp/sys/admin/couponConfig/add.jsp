	...
	<div class="easyui-tabs" id="tabActivity" style="width:100%">
		 <div title="活动配置" style="padding:10px;text-align:center">
		 	<form id="couponConfigForm" method="post">
				<table class="tableForm" style="width:75%;">
					...
					<tr>
						...
						<td colspan="3" style="text-align:left">
							...
							" class="easyui-combobox" style="width: 200px;" prompt="如为空，则先配置优惠券..."/>
						</td>
					</tr>
					<tr>
						<th>活动标题：</th>
						<td colspan="3">
							<input name="title" value="${couponActivityVo.title}"
								class="easyui-validatebox textbox" data-options="required:true,validType:'length[1,50]'"
								style="width: 650px;" />
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<th rowspan="4" style="width:50px;text-align:center">
							<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-large-picture',size:'large',iconAlign:'top'" 
								onclick="uploadImg('img','url','dc-test')" id="uploadImg" style="margin-bottom:10px;">上传活动图</a>
						</th>
						<td rowspan="4">
					...
					<tr>
						...
						<td colspan="3" style="text-align:left;">
							...
							<input  id="provinceId" name="provinceId" value="${couponActivityVo.provinceId}" 
							...
							" class="easyui-combobox" style="width: 130px;" prompt="-请选择省份-"/>
							<input  id="cityId" name="cityId" value="${couponActivityVo.cityId}" 
							...
							onSelect:function(record){
								...
								$('#cityCode').val(record.regionCode);
								$('#area').val('');
								$('#areaId').combobox('clear');
								$('#areaId').combobox('reload','<%=request.getContextPath()%>/sys/region/getList?parentId='+record.regionId);
							}
							" class="easyui-combobox" style="width: 130px;" prompt="-请选择城市-"/>
							<input  id="areaId" name="areaId" value="${couponActivityVo.areaId}" 
							...
							" 
							class="easyui-combobox" style="width: 130px;" prompt="-请选择区域-"/>
					</tr>
					<tr>
						<th>活动内容：</th>
						<td colspan="3">
				...
				<input id="province" name="province" type="hidden" value="${couponActivityVo.province}"/>
				<input id="city" name="city" type="hidden" value="${couponActivityVo.city}"/>
				<input id="cityCode" name="cityCode" type="hidden" value="${couponActivityVo.cityCode}"/>
				<input id="area" name="area" type="hidden" value="${couponActivityVo.area}"/>
			</form>
		</div>
	</div> 
</div>