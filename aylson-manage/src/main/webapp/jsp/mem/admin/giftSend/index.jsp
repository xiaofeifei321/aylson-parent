<%@ page language="java" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<jsp:include page="/resources/inc/meta.jsp"></jsp:include>
<jsp:include page="/resources/inc/easyui.jsp"></jsp:include>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
</head>
<body class="easyui-layout" fit="true">
	<!-- 角色管理 查询 --> 
	<div region="north"  id="giftSendArea" style="white-space: nowrap;padding: 5px; height: 50px;">
		<div style="margin-bottom: 5px">
			<form id="giftSendSearchForm"  method="post">
				<table class="table_content"   border="0" >
					<tr>
						<td class="tar" >收货人：</td>
						<td class="tal" >
							<input class="easyui-textbox" name="consigneeLike" prompt="模糊查询"/>
						</td>
						<td class="tar" >手机号码：</td>
						<td class="tal" >
							<input class="easyui-textbox" name="consigneePhoneLike" prompt="模糊查询"/>
						</td>
						<td class="tar" >提交时间：</td>
						<td class="tal" >
							<input class="easyui-datebox" name="createTime" />
						</td>
						<td class="tar" >更新时间：</td>
						<td class="tal" >
							<input class="easyui-datebox" name="updateTime" />
						</td>
					    <td style="padding-left:20px">
							<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-search'" style="width:80px" onclick="doSearch()">搜索</a>
							<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-clear'" style="width:80px" onclick="reset()">重置</a>
						</td> 
					</tr>
				</table>
			</form>
		</div>
	</div> 
	<div data-options="region:'center'"  border="false" style="overflow: hidden;width:85%">
    	<table id="datagrid"></table>
    </div>
</body>
<script type="text/javascript" charset="UTF-8" src="<%=request.getContextPath()%>/resources/js/aylson/mem/giftSend.js?date=2017022806"></script>
</html>