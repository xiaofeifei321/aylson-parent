package com.aylson.dc.client.controller;
...
public class ClientController extends BaseController {
	...
	public Result register(MemAccountVo memAccountVo) {
		...
		try{
			if(memAccountVo.getMemberType() == null){memAccountVo.setMemberType(MemberType.DESIGN);}
...
//		Result result = null;
//		try{
//			result = this.projectInfoService.confirmProjectInfo(projectId,"官网");
//		}catch(Exception e){
//			e.printStackTrace();
//			result.setError(ResultCode.CODE_STATE_500, "系统繁忙，请稍候再试或联系管理员");
//		}
...
//	}
	...
	public Result modifyPwd(String mobilePhone, String validCode, String newPwd, String confirmPwd) {
		Result result = null;
		try{
			result = this.memAccountService.modifyPwd(mobilePhone, validCode, newPwd, confirmPwd, null);
		}catch(Exception e){
			e.printStackTrace();
			result.setError(ResultCode.CODE_STATE_500, "系统繁忙，请稍候再试或联系管理员");
		}
	...
	public Result addMyCustomer(Integer projectClientId) {
		...
		try{
			...
			if(MemClientStatus.ADDED == projectClientVo.getStatus().intValue()){
				result.setError(ResultCode.CODE_STATE_4006, "已经是我的客户了");
				return result;
			}
