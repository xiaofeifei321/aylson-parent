	...
	$(function() { 
		datagrid = $('#datagrid').datagrid({
			method:'get',
			url : projectName+'/cfdb/appConfig/admin/list?v_date=' + new Date(),
			pagination : true,
			pageSize : 20,
...
//			},
	...
	function add(obj){
		...
		win = $("<div></div>").dialog({
			...
			height:'60%',
			modal:true,
			href:projectName+'/cfdb/appConfig/admin/toAdd',
			onClose:function(){
				$(this).dialog("destroy");
			},
			buttons:[{
			    ...
			    handler:function(){
				    	$("#appConfigConfigForm").form('submit',{
				    		 type:'POST',
				    		 url : projectName+'/cfdb/appConfig/admin/add',
	...
	function edit(id){
		win = $("<div></div>").dialog({
			...
			maximizable:true,
			modal:true,
			href:projectName+'/cfdb/appConfig/admin/toEdit?id='+id,
			onClose:function(){
		    		$(this).dialog("destroy");
		    },
			buttons:[{
				    ...
				    handler:function(){
					    	$("#appConfigConfigForm").form('submit',{
					    		 type:'POST',
					    		 url : projectName+'/cfdb/appConfig/admin/update',
					    		 success:function(responseData){
					    			 win.dialog('destroy');
					    			 if(responseData){
					    				var data = $.parseJSON(responseData);
	...
	function del(id){
		$.messager.confirm("提示","确定删除此记录吗？",function(r){
			if(r){
				$.ajax({
					type:"POST",
					url:projectName+'/cfdb/appConfig/admin/deleteById?id=' + id,
					dataType:"json",
